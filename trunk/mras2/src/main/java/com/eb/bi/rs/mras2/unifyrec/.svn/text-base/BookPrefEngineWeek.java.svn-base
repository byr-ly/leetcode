package com.eb.bi.rs.mras2.unifyrec;
...
import com.eb.bi.rs.mras2.unifyrec.ComprehensivePrefRecResult.ComprehensivePrefRecResultReducer;
import com.eb.bi.rs.mras2.unifyrec.ForceRecClassPref.ForceRecClassPrefReducer;
import com.eb.bi.rs.mras2.unifyrec.PrefRecResultMerge.PrefRecResultMergeMapper;
import com.eb.bi.rs.mras2.unifyrec.PrefRecResultMerge.PrefRecResultMergeReducer;
...
import com.eb.bi.rs.mras2.unifyrec.UserBookScoreTools.User_ReadHistoryMapper;
import com.eb.bi.rs.mras2.unifyrec.UserClassPref.UserClassPrefDriver;
import com.eb.bi.rs.mras2.unifyrec.UserClassPref.UserClassPrefReducer;
import com.eb.bi.rs.mras2.unifyrec.UserPrefAverage.UserPrefAverageMapper;
import com.eb.bi.rs.mras2.unifyrec.UserPrefAverage.UserPrefAverageReducer;
import com.eb.bi.rs.mras2.unifyrec.UserPrefGenerator.UserPrefGeneratorMapper;
...
import com.eb.bi.rs.mras2.unifyrec.UserReadClasses.UserReadClassesDriver;
import com.eb.bi.rs.mras2.unifyrec.UserReadClasses.UserReadClassesMapper;
import com.eb.bi.rs.mras2.unifyrec.UserReadClasses.UserReadClassesReducer;
import com.eb.bi.rs.mras2.unifyrec.UserReadHistory.UserReadHistoryMapper;
import com.eb.bi.rs.mras2.unifyrec.UserReadHistory.UserReadHistoryReducer;
 ...
 */
public class BookPrefEngineWeek extends Configured implements Tool {
    public static void main(String[] args) throws Exception {
        PluginUtil.getInstance().init(args);
        Logger log = PluginUtil.getInstance().getLogger();
        Date dateBeg = new Date();

        int ret = ToolRunner.run(new BookPrefEngineWeek(), args);
    ...
    public int run(String[] strings) throws Exception {
        ...
        prefAllPath = config.getParam("user_pref_all", "/home/recsys/data/recsys_data/attribute_engine/input/user_pref_all");
...
//        }
        ...
        job = Job.getInstance(conf, "unifyrec_prefengine_user-pref-generator");
...
//			DistributedCache.addCacheFile(URI.create(status[i].getPath().toString()), conf);
        ...
        job.setNumReduceTasks(config.getParam("user_vector_generator_reduce_task_num", 30));
        job.setMapOutputKeyClass(Text.class);
        ...
        String inputPath = config.getParam("user_bookid_all", "/home/recsys/data/recsys_data/attribute_engine/input/user_bookid_all");
        ...
        job = Job.getInstance(conf, "unifyrec_prefengine_user-read-classes");
        ...
        job.setNumReduceTasks(config.getParam("user_read_class_reduce_task_num", 30));
        ...
        job = Job.getInstance(conf, "unifyrec_prefengine_user-read-history");
        job.setJarByClass(BookPrefEngineWeek.class);
        ...
        job = Job.getInstance(conf,"unifyrec_prefengine_user-class-pref");
        ...
        userPrefVector = config.getParam("user_pref_vector", "/user/hadoop/liujie/test/output/user_pref_vector");
        bookClasses = config.getParam("user_read_classes", "/user/hadoop/liujie/test/output/user_read_classes");
        outputDir = config.getParam("forcerec_classpref_result", "/home/recsys/data/recsys_data/output/forcerec_classpref_result");
...
//            System.out.println(fs[i].getPath().toString() + " has been add into distributedCache");
//        }
        job = Job.getInstance(conf, "unifyrec_prefengine_force-rec-class-pref");

        status = FileSystem.get(conf).globStatus(new Path(manRecomBooks));
        for (int i = 0; i < status.length; i++) {
			/*DistributedCache修改点*/
            job.addCacheFile(new Path(status[i].getPath().toString()).toUri());
...
//			DistributedCache.addCacheFile(URI.create(status[i].getPath().toString()), conf);
        ...
        status = FileSystem.get(conf).globStatus(new Path(femaleRecomBooks));
        for (int i = 0; i < status.length; i++) {
			/*DistributedCache修改点*/
            job.addCacheFile(new Path(status[i].getPath().toString()).toUri());
...
//			DistributedCache.addCacheFile(URI.create(status[i].getPath().toString()), conf);
        ...
        status = FileSystem.get(conf).globStatus(new Path(publishRecomBooks));
        for (int i = 0; i < status.length; i++) {
			/*DistributedCache修改点*/
            job.addCacheFile(new Path(status[i].getPath().toString()).toUri());
...
//			DistributedCache.addCacheFile(URI.create(status[i].getPath().toString()), conf);
        ...
        userPrefVector = config.getParam("user_pref_vector", "/user/hadoop/liujie/test/output/user_pref_vector");
        bookClasses = config.getParam("user_read_classes", "/user/hadoop/liujie/test/output/user_read_classes");
        outputDir = config.getParam("comprehensive_prefrec_result", "/user/hadoop/liujie/test/output/comprehensive_prefrec_result");
...
//        }
        ...
        job = Job.getInstance(conf, "unifyrec_prefengine_comprehensive-pref-recommend");

        status = FileSystem.get(conf).globStatus(new Path(bookClassifierScores));
        for (int i = 0; i < status.length; i++) {
			/*DistributedCache修改点*/
            job.addCacheFile(new Path(status[i].getPath().toString()).toUri());
...
//			DistributedCache.addCacheFile(URI.create(status[i].getPath().toString()), conf);
        ...
        userPrefVector = config.getParam("user_pref_vector", "/user/hadoop/liujie/test/output/user_pref_vector");
        bookClasses = config.getParam("user_read_classes", "/user/hadoop/liujie/test/output/user_read_classes");
        outputDir = config.getParam("man_prefrec_result", "/user/hadoop/liujie/test/output/man_prefrec_result");
...
//        }
        ...
        job = Job.getInstance(conf, "unifyrec_prefengine_man-pref-recommend");
...
//			DistributedCache.addCacheFile(URI.create(status[i].getPath().toString()), conf);
        ...
        userPrefVector = config.getParam("user_pref_vector", "/user/hadoop/liujie/test/output/user_pref_vector");
        bookClasses = config.getParam("user_read_classes", "/user/hadoop/liujie/test/output/user_read_classes");
        outputDir = config.getParam("female_prefrec_result", "/user/hadoop/liujie/test/output/female_prefrec_result");
        bookClassifierScores = config.getParam("classifier_output_d", "/home/recsys/data/recsys_data/attribute_engine/input/classifier");
...
//            System.out.println(fs[i].getPath().toString() + " has been add into distributedCache");
//        }
        job = Job.getInstance(conf, "unifyrec_prefengine_female-pref-recommend");
...
//			DistributedCache.addCacheFile(URI.create(status[i].getPath().toString()), conf);
        ...
        userPrefVector = config.getParam("user_pref_vector", "/user/hadoop/liujie/test/output/user_pref_vector");
        bookClasses = config.getParam("user_read_classes", "/user/hadoop/liujie/test/output/user_read_classes");
        outputDir = config.getParam("publish_prefrec_result", "/user/hadoop/liujie/test/output/publish_prefrec_result");
        bookClassifierScores = config.getParam("classifier_output_d", "/home/recsys/data/recsys_data/attribute_engine/input/classifier");
...
//        }
        ...
        job = Job.getInstance(conf, "unifyrec_prefengine_publish-pref-recommend");
...
//			DistributedCache.addCacheFile(URI.create(status[i].getPath().toString()), conf);
        ...
        outputPath = config.getParam("prefrec_merge_result", "/user/hadoop/liujie/test/output/prefrec_merge_result");
        ...
        job = Job.getInstance(conf, "unifyrec_prefengine_prefrec-result-merge");
        job.setJarByClass(BookPrefEngineDay.class);

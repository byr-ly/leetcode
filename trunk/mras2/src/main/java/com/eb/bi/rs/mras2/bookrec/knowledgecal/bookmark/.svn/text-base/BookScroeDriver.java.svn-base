...
public class BookScroeDriver extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		int maxSplitSizejob1 = config.getParam("tagdict_max_split_size_job1",
				64);
		...
		conf.set("mapred.max.split.size",
				String.valueOf(1024 * 1024 * maxSplitSizejob1));
		...
		// 设置输出地址
		FileOutputFormat.setOutputPath(job, new Path(workhdfspath
				+ "/bookreadnum"));
		...
		MultipleInputs.addInputPath(job, new Path(inputhdfsdir2),
				KeyValueTextInputFormat.class, BookChargetypeMapper.class);
		MultipleInputs.addInputPath(job, new Path(inputhdfsdir1),
				KeyValueTextInputFormat.class, BookTotalChMapper.class);// workhdfspath
																		// +
																		// "/bookCHnum/part-*"

		// 设置输出地址
		FileOutputFormat.setOutputPath(job,
				new Path(workhdfspath + "/bookTYPE"));
		...
		// -----
		// 添加内存加载图书|type
		log.info("mapred.cache.localFiles No.1"
				+ conf.getStrings("mapred.cache.localFiles"));
		log.info("mapred.cache.localFiles No.1"
		...
		FileStatus[] status1 = fs1.globStatus(new Path(workhdfspath
				+ "/bookTYPE/part-*"));
		for (int i = 0; i < status1.length; i++) {
			job.addCacheFile(new Path(status1[i].getPath().toString()).toUri());
			log.info("book_type file: " + status1[i].getPath().toString()
					+ " has been add into distributed cache");
		}

		log.info("mapred.cache.localFiles No.2"
				+ conf.getStrings("mapred.cache.localFiles"));
		log.info("mapred.cache.localFiles No.2"
				+ conf.getStrings("mapred.cache.files"));

		FileStatus[] status2 = fs1.globStatus(new Path(workhdfspath
				+ "/bookreadnum/part-*"));
		for (int i = 0; i < status2.length; i++) {
			job.addCacheFile(new Path(status2[i].getPath().toString()).toUri());
			log.info("book_readnum file: " + status2[i].getPath().toString()
					+ " has been add into distributed cache");
		}

		log.info("mapred.cache.localFiles No.3"
				+ conf.getStrings("mapred.cache.localFiles"));
		log.info("mapred.cache.localFiles No.3"
				+ conf.getStrings("mapred.cache.files"));
		...
		// 设置输入地址
		FileInputFormat.setInputPaths(job, new Path(workhdfspath
				+ "/userbookMpoint/part-*"));
		// 设置输出地址
		FileOutputFormat.setOutputPath(job, new Path(workhdfspath
				+ "/userbookscore"));

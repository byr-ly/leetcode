...
public class ComprehensiveBookVector {
    ...
    public ComprehensiveBookVector(HashMap<String, Double> bookScores, String bookID, String classId, String ifNew, String ifEditFame,
               String chargeType, String ifFinish, String ifHotRec, String sexId, String if_zjhot) {
        this.bookID = bookID;
        ...
        pack_weight = 0;
        //浙江网关热书
        zjhot_weight = this.getIntegerValue(if_zjhot) == 1 ? 1 : 0;
    }
...
//                charge_weight + "|" + sale_weight + "|" + pack_weight);
    ...
    public boolean getZjhotWeight(UserVector user) {
        //将if_zjhot=‘1’的图书分类匹配用户前三分类偏好
        if (this.zjhot_weight == 1) {
            //属于用户喜欢的前三个分类
            if (this.classId.equals(user.class1_id) || this.classId.equals(user.class2_id) ||
                    this.classId.equals(user.class3_id)) {
                if(SeparatePrefRecResultReducer.debug) {

...
public class SelfSignedRecReducer extends
	...
	protected void reduce(Text key, Iterable<Text> value, Context context)
				...
				if (classidAndbigMap.containsKey(classid)) {
					...
					for (String s : list) {
						if (!s.equals(key.toString())
								&& selfSignMap.get(s).equals("1")
								&& !selfSignList.contains(s)
								&& !selfSignFiller.contains(s)) {
							selfSignList.add(new BookInfo(s, ""));
							//selfSignFiller.add(s);
							classidFiller++;
						}
						//分类补白自签书的数量不能多于classidFillerNum
						if(classidFiller >= classidFillerNum){
							break;
						}
		...
		// 第二次补白 大类补白
//		if (selfSignList.size() < 4) {
//			if (bigMap.containsKey(key.toString())) {
//				String bigid = bigMap.get(key.toString());
//
//				if (classidAndbigMap.containsKey(bigid)) {
//
//					ArrayList<String> list = classidAndbigMap.get(bigid);
//					Collections.shuffle(list);
//					for (String s : list) {
//						if (!s.equals(key.toString())
//								&& selfSignMap.get(s).equals("1")
...
//		}
		...
		for (BookInfo b : selfSignList) {
			System.out.println(b.getId() + "  chenyuxiao   " + b.getScore());
		...
		if (recResultSelfSignNum == 0 && selfSignList.size() == 0) {
			...
			// System.out.println("-------------no--------selfBook--------");
	...
	protected void setup(Context context) throws IOException,
		...
		int selfIndex = conf.getInt("self.index", 7);
		classidFillerNum = conf.getInt("classid.filler.num", 2);
				...
				if (cacheFiles[i].toString().contains(cacheInputPath)) {
					while ((line = in.readLine()) != null) {
						...
						if (fields.length >= 8) {
							...
							String selfSign = fields[selfIndex];
							classidMap.put(bookId, classid);
							bigMap.put(bookId, bigClass);
...
//							if (classidAndbigMap.containsKey(bigClass)) {
//								classidAndbigMap.get(bigClass).add(bookId);
//							} else {
//								classidAndbigMap.put(bigClass,
//										new ArrayList<String>());
//								classidAndbigMap.get(bigClass).add(bookId);
//							}
						}

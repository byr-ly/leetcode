...
import com.eb.bi.rs.mras2.bookrec.corelationrec.selfsign.SelfSignedRecMapper;
import com.eb.bi.rs.mras2.bookrec.corelationrec.selfsign.SelfSignedRecReducer;
...
public class SelfSignedRecDriver extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		 ...
		 * 输入： 1.输入路径：推荐结果数据 
		 *      2.缓存文件：图书分类信息 
		 * 输出： 将自签书提前到前十位的推荐结果数据
		 * 功能描述: 1.筛选规则 推荐结果50本结果中前10本中要有N本自签书推荐库中的图书，暂定N=4 
		 *         2.补白策略先在原来推荐结果里筛选，若不够N,则去classid里补白自签书,若还不够N,则去big_class里补够N,
		 *           若还不够N，则自签书数量就为补白出来的数量
		 ****************************************************/
		log.info("=================================================================================");
		start = System.currentTimeMillis();

		String recResultInputPath = config.getParam(
				"read_corelation_book_recommend_selfsign_path", "");
		String cacheInputPath = config.getParam("bookinfo_cache_input", "");
		String recResultOutputPath = config.getParam(
				"read_corelation_book_recommend_path", "");

		int reduceNum = config.getParam("reduce_num", 100);
		int selfIndex = config.getParam("self_index", 7);
		int classidFillerNum = config.getParam("classid_filler_num", 2);
		
		conf = new Configuration(getConf());
		conf.set("cache.input.path", cacheInputPath);
		conf.setInt("self.index", selfIndex);
		conf.setInt("classid.filler.num", classidFillerNum);

		job = new Job(conf,
				"CorelationRecommend-ReadAlsoRead-read_selfsign_book_recommend");

		status = FileSystem.get(conf).globStatus(
				new Path(cacheInputPath + "/*"));
		for (int i = 0; i < status.length; i++) {
			// DistributedCache.addCacheFile(URI.create(status[i].getPath().toString()),
			// conf);
			job.addCacheFile(URI.create(status[i].getPath().toString()));
			log.info("cache data input file: " + status[i].getPath().toString()
					+ " has been add into distributed cache");
		}
		...
		String recResultInputPath2 = config.getParam(
				"read_self_recresult_for_book_without_cooccurrenceinfo_path",
				"");
		String recResultOutputPath2 = config.getParam(
				"read_recresult_for_book_without_cooccurrenceinfo_path", "");
		job = new Job(
				conf,
				"CorelationRecommend-ReadAlsoRead-read_selfsign_book_recommend_without_cooccurrenceinfo");

...
public class CalculationOfRelationInCacheDriver extends BaseDriver{
	...
	public int run(String[] args) throws Exception {
		...
		for(FileStatus status: status1){
			job.addCacheFile(URI.create(status.getPath().toString()));
			log.info("book_similar file: " + status.getPath().toString() + " has been add into distributed cache");
		}

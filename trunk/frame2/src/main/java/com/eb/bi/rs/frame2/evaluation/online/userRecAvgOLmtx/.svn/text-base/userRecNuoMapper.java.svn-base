 ...
 */
public class userRecNuoMapper extends Mapper<Object, Text, Text, Text> {
    ...
    输入数据:
        用户推荐位订购图书数（去重）表
        msisdn用户 | 推荐位 | 用户订购的推荐位图书数（去重） | 时间

    输出数据:
        key: msisdn用户 | 推荐位 | 标识
        value: "Nuo" | 用户订购的推荐位图书数（去重）
    */
    public void map(Object text, Text value, Context context) throws IOException, InterruptedException {
        ...
        if (fields.length == 4) {
        	if (fields[0] != null && fields[1] != null &&
            		...
            		for(Map.Entry<String, String> entry : fieldNumber.entrySet() ){
                		...
                		for(int i=0;i<number.length;i++){
                			if(fields[0].substring(digit-1,digit).equals(number[i])){
                				context.write(new Text(fields[0] + "|" + fields[1] + "|" + entry.getValue()), new Text("Nuo|" + fields[2]));
                				flag = false;
                			}
                	}
                }
            	...
            	if(flag==true){
            		context.write(new Text(fields[0] + "|" + fields[1] + "|" + "0"), new Text("Nuo|" + fields[2]));
            	}
           }
        }
    }

...
public class CooccurenceDriver extends Configured implements Tool{
	...
	public int run(String[] args) throws Exception { 
		...
		return ConfidenceLevelManager.ConfidenceLevel(args, logger, getConf(), 
					...
					config.getParam("tagdict_max_split_size_job", 64),  config.getParam("KULC_threshold", "0.03") , config.getParam("IR_threshold","0.7"), 
					config.getParam("min_num", ""),config.getParam("neighbour_num", "0"),config.getParam("max_item_num", "5000"),config.getParam("hive_input_field_delimiter", "\\|"));

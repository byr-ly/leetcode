package com.eb.bi.rs.mras.bookrec.corelationrec;
...
import com.eb.bi.rs.mras.bookrec.corelationrec.CommonUtil.IndicatorFilterMapper;
import com.eb.bi.rs.mras.bookrec.corelationrec.CommonUtil.JoinClassFrequencyReducer;
import com.eb.bi.rs.mras.bookrec.corelationrec.CommonUtil.JoinClassMapper;
import com.eb.bi.rs.mras.bookrec.corelationrec.CommonUtil.JoinFrequncyMapper;
import com.eb.bi.rs.mras.bookrec.corelationrec.CommonUtil.ReadCorelationBookRecMapper;
import com.eb.bi.rs.mras.bookrec.corelationrec.CommonUtil.ReadCorelationBookRecReducer;
import com.eb.bi.rs.mras.bookrec.corelationrec.CommonUtil.ReadRecForBookWithoutCooccurrenceInfoMapper;
import com.eb.bi.rs.mras.bookrec.corelationrec.CommonUtil.ReadRecForBookWithoutCooccurrenceInfoReducer;
import com.eb.bi.rs.mras.bookrec.corelationrec.CommonUtil.TextPair;
...
public class ReadCorelationRecDriver extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		conf.setInt("deep.read.threshold", config.getParam("read_deep_read_threshold", 100));		
		conf.set("book.info.path", bookInfoPath);
		conf.set("book.deep.read.frequency.path", bookDeepReadFrequencyPath);
		...
		status = FileSystem.get(conf).globStatus(new Path(bookDeepReadFrequencyPath + "/part-*"));
		for (int i = 0; i < status.length; i++) {
			DistributedCache.addCacheFile(URI.create(status[i].getPath().toString()), conf);
		...
		float firstUpperBound = config.getParam("first_upper_bound", 0.35f);
		...
		conf.setFloat("first.lower.bound", config.getParam("first_lower_bound", 0.1f));
		conf.setFloat("first.upper.bound", config.getParam("first_upper_bound", 0.35f));
		conf.set("book.series.path", bookSeriesPath);

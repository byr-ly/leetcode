...
import com.eb.bi.rs.mras.bookrec.corelationrec.OrderFilterRead.OrderRecResultMapper;
import com.eb.bi.rs.mras.bookrec.corelationrec.OrderFilterRead.OrderRecReultFilterReducer;
import com.eb.bi.rs.mras.bookrec.corelationrec.OrderFilterRead.ReadRecResultMapper;
...
public class OrderCorelationResultFilterDriver extends Configured implements Tool{
	 ...
	 * ********************************************************************************************/
	 ...
	 /*************************************************************************************************
		...
		**                  阅读还阅读推荐结果
		** 输出：
		**		过滤了阅读还阅读top20的订购还订购推荐结果
		** 功能描述：
	    ** 	      过滤订购还订购关联结果，使其满足：
		**		      同一用户的阅读还阅读推荐结果的top20不在订购还订购推荐结果里
		**
	 *************************************************************************************************/ 
     log.info("=================================================================================");
     ...
     conf.setInt("order.corelation.recommend.number", config.getParam("order_corelation_recommend_number", 70));
     conf.setInt("order.filter.read.topN", config.getParam("order_filter_read_top_N", 20));
      
     job = new Job(conf, "order-without-cooccurrence-corelation-rec-result-filter");
	 ...
	 *************************************************************************************************/ 
     ...
     conf.setInt("order.corelation.recommend.number", config.getParam("order_corelation_recommend_number", 50));
     conf.setInt("order.filter.read.topN", config.getParam("order_filter_read_top_N", 20));
      
     job = new Job(conf, "order-corelation-rec-result-filter");
     job.setJarByClass(getClass());

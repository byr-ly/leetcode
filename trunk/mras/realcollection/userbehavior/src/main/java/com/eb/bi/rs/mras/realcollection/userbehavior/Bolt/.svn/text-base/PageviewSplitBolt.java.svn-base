 ...
 */
public class PageviewSplitBolt extends BaseBasicBolt {

    private static final long serialVersionUID = 1L;
    static Logger log = Logger.getLogger(PageviewSplitBolt.class);
    ...
    public void execute(Tuple input, BasicOutputCollector collector) {
        ...
        if (words.length >= 57) {
            ...
            collector.emit(StreamId.BROWSEDATA.name(), new Values(msisdn, time, bookId, pageType));
        } else {
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declareStream(StreamId.BROWSEDATA.name(),
                new Fields(FName.MSISDN.name(), FName.RECORDTIME.name(), FName.BOOKID.name(), FName.PAGETPYE.name()));
    }
}

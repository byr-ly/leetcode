...
public class ItemFilterDriver extends BaseDriver{
	...
	public int run(String[] args) throws Exception {
		...
		if(toFilterInputPaths == null){
			...
			for(int i = 0; i < toFilterInputPathArr.length; ++i) {
				MultipleInputs.addInputPath(job, new Path(toFilterInputPathArr[i]), TextInputFormat.class, Item2FilterMapper.class);
			}
		}		
		...
		if(filterInputPaths == null){
			...
			for (int i = 0; i < filterInputPathArr.length; i++) {
				MultipleInputs.addInputPath(job, new Path(filterInputPathArr[i]), TextInputFormat.class, ItemFilterMapper.class);
			}
		}	
		...
		job.setReducerClass(ItemFilterReducer.class);

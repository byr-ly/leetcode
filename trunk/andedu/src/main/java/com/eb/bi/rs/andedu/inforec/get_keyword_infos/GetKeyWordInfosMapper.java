 ...
 */
public class GetKeyWordInfosMapper extends Mapper<Object, Text, Text, Text> {
    /***
     * @param key
     * @param value
     *
    ...
    protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
        if (value.toString().trim().isEmpty() || value.toString().split("\t").length < 2) {
            System.out.println("GetKeyWordInfosMapper bad line : " + value.toString());
            return;
        }
        String infosKey = value.toString().split("\t")[0];
...
//        }
        ...
        for (String perWordWeight : valuesFields) {
            ...
            context.write(new Text(word), new Text(infosID + "," + weight));
        }
    }
}

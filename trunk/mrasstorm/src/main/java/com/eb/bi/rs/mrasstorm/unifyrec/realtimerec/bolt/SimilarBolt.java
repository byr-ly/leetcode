...
public class SimilarBolt extends BaseBasicBolt {
	...
	public void prepare(Map stormConf, TopologyContext context) {
		...
		String[] users = appConfig.getParam("users").split(",");
	...
	public void execute(Tuple input, BasicOutputCollector collector) {
		...
		String edition = input.getStringByField("edition");
		PrintHelper.print("SimilarBolt receive userid:" + user + ">> edition_id:" + edition);
		...
		try {
			...
			if(userSet.contains(user)){
				collector.emit("MergeBoltB", new Values(user, edition, result));
			} else {
				collector.emit("MergeBoltA", new Values(user, edition, result));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		declarer.declareStream("MergeBoltA", new Fields("user", "edition", "book_scores"));
		declarer.declareStream("MergeBoltB", new Fields("user", "edition", "book_scores"));
	}

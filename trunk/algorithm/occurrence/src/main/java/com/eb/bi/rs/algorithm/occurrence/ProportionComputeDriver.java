...
public class ProportionComputeDriver extends Configured  implements Tool{
	...
	public ProportionComputeDriver(Logger log,String inputPath, String outputPath, Configuration conf, int reduceNum){
		m_log = log;
	...
	public int run(String[] arg0) throws Exception {
		...
		job.setJarByClass(ProportionComputeDriver.class);
		//设置输入地址
		FileInputFormat.setInputPaths(job, new Path(m_inputPath));
		//设置输出地址
		FileOutputFormat.setOutputPath(job, new Path(m_outputPath));
		//设置M-R
		job.setMapperClass(ProportionComputeMapper.class);
		job.setNumReduceTasks(m_reduceNum);
		job.setReducerClass(ProportionComputeReducer.class);
		//设置输入/输出格式
